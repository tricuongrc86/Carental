 ...
 * @author TriCuong
 */
@WebServlet(name = "ReSendCodeServlet", urlPatterns = {"/ReSendCodeServlet"})
public class ReSendCodeServlet extends HttpServlet {

    private static final Logger LOGGER = Logger.getLogger(ReSendCodeServlet.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "";
        try {
            if (!session.getAttribute("ROLE").equals("New")) {
                url = (String) context.getAttribute("errorPage");
            }
            if (session.getAttribute("ROLE").equals("New")) {
                int code = ThreadLocalRandom.current().nextInt(10000, 99999);
                String email = session.getAttribute("EMAIL").toString();
                RegistrationTblDAO dao = new RegistrationTblDAO();
                dao.updateVerifyCode(email, String.valueOf(code));
